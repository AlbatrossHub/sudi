...
class OwaCustomer360(models.AbstractModel):
    ...
    def get_timeline_data(self, partner_id, date_from=None, date_to=None, limit=300):
        ...
        if not partner:
            return {'partner': None, 'events': []}
        # This aggregates cross-app data (orders, invoices, CRM, helpdesk).
        # Block non-internal callers, and read the business apps WITHOUT sudo so
        # each app's record rules apply — previously every read was sudo'd, so
        # any internal user could pull any partner's financials/CRM/helpdesk
        # past their own access. (#F125)
        if not self.env.su and self.env.user.share:
            raise AccessError(_("Access denied."))

        def _rr_search(model_name, domain, **kw):
            """Search honouring the caller's record rules; return empty if the
            user has no access to the model at all, so the timeline never leaks
            cross-app records the agent isn't entitled to. (#F125)"""
            try:
                return self.env[model_name].search(domain, **kw)
            except AccessError:
        ...
        if 'helpdesk.ticket' in self.env:
            tickets = _rr_search('helpdesk.ticket', [
                ('partner_id', '=', partner.id),
                ('create_date', '>=', dt_from), ('create_date', '<=', dt_to),
            ], order='create_date desc', limit=limit)
        ...
        return {
            'partner': {
                ...
                # Odoo 19 has no res.partner.mobile field — getattr keeps this
                # safe on both trees (v18 returns it, v19 returns ''). (gotcha #1)
