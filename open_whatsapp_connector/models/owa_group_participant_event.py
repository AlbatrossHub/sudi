...
# These labels live in a module-level constant, which Python evaluates at
# import time — when there is no active cursor/language. Calling _() there only
# logs "no translation language detected, skipping translation" and never
# actually translates. LazyTranslate defers the lookup to __str__ (i.e. when the
# label is rendered at request time), the correct idiom for translatable
...
_ACTION_LABEL = {
    'add': _lt("joined"),
    'remove': _lt("left"),
    'promote': _lt("was promoted to admin"),
    'demote': _lt("was demoted from admin"),
}
