...
class OwaSlashCommand(models.Model):
    ...
    def _send_reply(self, account, from_number, body, skip_blacklist=False):
        """Helper: queue a text reply via owa.message + cron trigger.

        ``skip_blacklist`` lets a confirmation (e.g. the /stop opt-out notice)
        reach a number that was just added to the blacklist."""
    ...
    def _handle_stop(self, account, from_number, body, channel):
        ...
        # skip_blacklist so this confirmation isn't blocked by the entry we
        # just created.
        self._send_reply(
            account, from_number,
    ...
    def _handle_start(self, account, from_number, body, channel):
        """Opt the sender back in: remove from the blacklist and confirm."""
        self.env['owa.blacklist'].sudo().remove_from_blacklist(from_number)
        # skip_blacklist guards against a redelivery racing the un-blacklist.
        self._send_reply(
            account, from_number,
            _("Welcome back! You've been opted in again."),
            skip_blacklist=True,
        )
