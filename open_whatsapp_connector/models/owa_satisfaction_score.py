...
class OwaSatisfactionScore(models.Model):
    ...
    def _send_csat_invite(self, channel):
        ...
        if 'survey.survey' not in self.env:
            return False
        # Only send the explicitly-configured CSAT survey. Without this guard
        # the old search([], limit=1) shipped whatever survey sorted first
        # (e.g. an internal HR appraisal) to the customer. (#F131)
        account = channel.owa_account_id
        survey_id = account.csat_survey_id_int if account else 0
        if not survey_id:
            return False
        try:
            survey = self.env['survey.survey'].sudo().browse(survey_id).exists()
            if not survey:
                _logger.info(
                    "CSAT invite skipped: configured survey id %s no longer "
                    "exists (account %s)", survey_id, account.id)
                return False
            ...
            mail_message = self.env['mail.message'].sudo().create({
                'body': plaintext2html(_("Thanks for chatting with us! Please rate your experience: %s") % link),
