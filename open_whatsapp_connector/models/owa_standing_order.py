...
class OwaStandingOrder(models.Model):
    ...
    def _fire(self):
        self.ensure_one()
        qr = self.quick_reply_id
        # A standing order with no/empty template can never send; still advance
        # the schedule so the cron doesn't re-fire it on every tick.
        if not qr or not (qr.body or '').strip():
        ...
        if self.recipient_kind == 'broadcast_group' and self.broadcast_group_id:
            # Render PER recipient (variable substitution + placeholder strip)
            # inside the fan-out so {{tokens}} are never delivered verbatim.
            sent = self.broadcast_group_id.fanout('', quick_reply=qr)
            ...
            for member in self.contact_list_id.member_ids:
                ...
                try:
                    ...
                    body = qr.render_plain(record=member.partner_id or None)
            ...
            if mobile:
                try:
                    body = qr.render_plain(record=self.partner_id)
