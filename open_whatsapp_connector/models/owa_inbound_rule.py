...
class OwaInboundRule(models.Model):
    ...
    def _build_create_vals(self, partner, body, channel, sender_phone=None):
        ...
        # point at a live record. Both crm.lead.team_id and sale.order.team_id
        # are crm.team; only helpdesk.ticket.team_id is a helpdesk.team.
        team_model = (
            'crm.team' if self.target_model in ('crm.lead', 'sale.order')
            else 'helpdesk.team')
        team_id = self._resolve_int_fk(team_model, self.target_team_id_int)
    ...
    def _evaluate_and_create(self, account, partner, body, channel=None,
        ...
        for rule in rules:
            ...
            try:
                # Savepoint so a failing create (e.g. a target ValidationError
                # or IntegrityError) rolls back cleanly instead of poisoning the
                # cursor for the NEXT rule's search and aborting the whole
                # inbound transaction.
                with self.env.cr.savepoint():
                    ...
                    # CRM bridge back-link (open_whatsapp_connector_crm): cross-
                    # link the captured lead and the conversation both ways. The
                    # reverse field only exists when the CRM glue addon is
                    # installed, so it is guarded. (#wa-crm-bridge)
                    if (rule.target_model == 'crm.lead'
                            and channel and channel.exists()):
                        try:
                            channel.sudo().crm_lead_id_int = rec.id
                            if 'wa_channel_id_int' in rec._fields:
                                rec.sudo().wa_channel_id_int = channel.id
                        except Exception:
                            _logger.exception(
                                "Inbound rule %s: crm back-link failed", rule.name)
                    ...
                    if rule.auto_reply_template:
                        ...
                        if mobile:
                            ...
                            # owa.message.body is a READONLY related field on
                            # mail_message_id, so writing 'body' directly is
                            # silently dropped and the queued reply is empty.
                            # Build the backing mail.message first (like every
                            # other outbound producer).
                            reply_mail = self.env['mail.message'].sudo().create({
                                'body': body_out or '',
