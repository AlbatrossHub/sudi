...
# Discuss renders @mentions and record references as <a href="/web#..."> anchors.
# Odoo's html2plaintext turns each into a "[N]" inline marker plus a trailing
# "[N] <url>" footnote, which then gets sent verbatim into the WhatsApp chat.
# Unwrap anchors to their visible text BEFORE converting, so the outbound message
...
class OwaMessage(models.Model):
    ...
    skip_blacklist = fields.Boolean(
        string="Bypass blacklist", default=False, copy=False,
        help="Internal: allow this outbound message past the opt-out blacklist "
             "(e.g. the opt-out/opt-in confirmation itself).")
    pending_report_id = fields.Many2one(
        'ir.actions.report', string="Pending PDF Report", copy=False,
        help="Internal: a notification rule deferred this report's PDF to send "
             "time so the triggering action (e.g. confirming a sale order) is "
             "not blocked by synchronous PDF rendering. The send cron renders "
             "and attaches it just before dispatch.")
    ...
    def _send_single_message(self):
        ...
        if self.wa_account_id._account_messaging_blocked():
            self._handle_error(
                'account',
                _("WhatsApp account is pending administrator approval"))
            return

        # Anti-ban throttle applies to EVERY send path, not just the queue:
        # if this account is inside a backoff window or already at a
        # per-minute/hour/day cap, do NOT transmit now — leave the message
        # 'outgoing' so the send-queue cron dispatches it once the window
        # frees. Without this, interactive sends (Discuss reply, Compose,
        # chatbot, rules) bypassed throttling entirely. (#throttle-all-paths)
        if self.state == 'outgoing' and self.wa_account_id._owa_is_throttled():
            _logger.info(
                "owa.message %s deferred: account %s is throttled",
                self.id, self.wa_account_id.id)
            return

        # Check blacklist (skip_blacklist lets the opt-out/opt-in confirmation
        # itself reach a just-blacklisted number).
        number = self.mobile_number_formatted or self.mobile_number
        if not self.skip_blacklist and self.env['owa.blacklist'].sudo().is_blacklisted(number):
        ...
        try:
            # Render any notification-rule PDF deferred to send time, so the
            # action that triggered the rule (sale confirm / invoice post /
            # delivery validation) was never blocked by wkhtmltopdf. (#async-notif)
            if self.pending_report_id:
                self._render_pending_report()
            body_text = _html_to_wa_text(self.body)
            attachments = (
            ...
            # If the body wasn't pre-rendered but a TEXT template is attached,
            # render it now. Messages created outside the normal send pipeline
            # (a direct data-import into owa.message, or an external RPC
            # integration) carry quick_reply_id but no rendered body — and
            # owa.message.body is a related field on mail_message_id.body, so
            # without a linked mail.message the body is empty. Previously these
            # failed "Empty message" because only button/list templates were
            # rendered here. (#text-template-fallback)
            if (not body_text and template
                    and template.template_type not in ('button', 'list')
                    and not attachments):
                rendered = template.render_body(record=self.whatsapp_partner_id or None)
                body_text = _html_to_wa_text(rendered or '')
            ...
            # Own-device sync: pre-claim this id so the fromMe ECHO of this very
            # message (WhatsApp delivers it back to the sidecar) is deduped at
            # the inbound webhook instead of double-posting. (#own-device-sync)
            if msg_uid:
                self.env['owa.inbound.dedupe'].sudo().claim(
                    msg_uid, self.wa_account_id.id)
    ...
    def _render_pending_report(self):
        """Render + attach a notification rule's deferred PDF report at SEND
        time (cron thread). Notification rules stamp ``pending_report_id``
        instead of rendering synchronously, so the action that triggered the
        rule (confirming a sale order, posting an invoice, validating a
        delivery) is never blocked by wkhtmltopdf. Best-effort: a render
        failure logs and the message still sends without the PDF. (#async-notif)
        """
        self.ensure_one()
        report = self.pending_report_id
        mm = self.mail_message_id
        try:
            if report and mm and mm.model and mm.res_id:
                pdf, _content_type = report.sudo()._render_qweb_pdf(
                    report.report_name, [mm.res_id])
                record = self.env[mm.model].sudo().browse(mm.res_id)
                attachment = self.env['ir.attachment'].sudo().create({
                    'name': '%s.pdf' % (record.display_name or report.report_name),
                    'datas': base64.b64encode(pdf),
                    'mimetype': 'application/pdf',
                    'res_model': mm.model,
                    'res_id': mm.res_id,
                })
                mm.sudo().write({'attachment_ids': [(4, attachment.id)]})
        except Exception:
            _logger.exception(
                "Deferred PDF render failed for owa.message %s", self.id)
        finally:
            if self.pending_report_id:
                self.sudo().pending_report_id = False
    ...
    def _send_cloud_message(self, account, number, body_text, attachments):
        ...
        try:
            ...
            if (template and template.template_type in ('button', 'list')
                    and not attachments):
                # Interactive buttons / list over Cloud — same renderer and
                # sender as the QR transport; CloudApi mirrors the client
                # signatures. (#cloud-interactive)
                msg_uid = self._send_interactive_template(
                    account._get_cloud_api(), number, template)
            elif attachments:
        ...
        # Own-device sync: pre-claim this id so the fromMe ECHO of this very
        # message is deduped at the inbound webhook, not double-posted.
        # (#own-device-sync)
        if msg_uid:
            self.env['owa.inbound.dedupe'].sudo().claim(
                msg_uid, self.wa_account_id.id)
    ...
    def _send_cron(self):
        ...
        Each message runs inside a savepoint so a DB-level error on one doesn't
        poison the whole batch, AND we commit after each message so a worker
        time-limit kill (``limit_time_real``) on a large queue can't roll back
        the entire batch. Each WhatsApp send is an irreversible API call that
        takes ~1-2s, so a queue of 100+ can exceed the worker timeout; if the
        worker is killed mid-batch without per-message commits, the whole
        transaction rolls back, nothing is persisted, and every subsequent tick
        re-attempts the same set and is killed again — the queue sticks forever.
        Incremental commit means transmitted messages are saved and the next
        tick resumes from where this one stopped. (#bulk-commit)

        Phase 26A: respects per-account throttle caps and backoff windows.
        """
        from datetime import timedelta
        from odoo.modules.module import current_test
        # Never commit mid-cron under tests: TransactionCase wraps each test in
        # one transaction that is rolled back at teardown, so a commit here
        # would leak fixtures across tests. This is the exact flag core
        # mail.mail.process_email_queue uses to gate its own auto_commit.
        ...
        for msg in messages:
            ...
            # Phase 26A: skip this tick while the account is in a rate-limit
            # backoff window or already at a per-minute/hour/day cap. Same
            # shared check every immediate send path uses. (#throttle-all-paths)
            ...
            # Persist this message's outcome (sent / error) before moving to the
            # next one, so a mid-batch worker kill never discards an
            # already-transmitted message or re-queues it for a double-send.
            if auto_commit:
                self.env.cr.commit()
    ...
    def action_open_edit_wizard(self):
        """Open the edit wizard pre-filled with this message's current text."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _("Edit WhatsApp Message"),
    ...
    def _process_status_update(self, msg_uid, status, account=None):
        """Update message state from sidecar status webhook.

        ``account`` (the webhook-authenticated owa.account) scopes the lookup:
        the same WhatsApp message id can exist as a row on several accounts in
        one DB (e.g. two of our numbers in the same group), so an unscoped
        search could write the wrong account's message. (#F012)"""
        domain = [('msg_uid', '=', msg_uid), ('message_type', '=', 'outbound')]
        if account:
    ...
    def _apply_inbound_reaction(self, msg_uid, sender_jid, emoji, account=None):
        ...
        ``account`` (the webhook-authenticated owa.account) scopes the lookup
        so a reaction can't be misrouted to a same-msg_uid row on another
        account. (#F012)
        """
        import json
        if not msg_uid or not sender_jid:
            return False
        domain = [('msg_uid', '=', msg_uid)]
        if account:
            domain.append(('wa_account_id', '=', account.id))
        message = self.search(domain, limit=1)
        if not message:
