...
class OwaBlacklist(models.Model):
    ...
    def add_to_blacklist(self, phone, reason=None):
        ...
        """
        formatted = wa_phone_format(self.env, phone) or phone
        # active_test=False so a previously-removed (archived) entry is found and
        # reactivated instead of hitting the UNIQUE(phone_formatted) constraint
        # when the same number opts out again.
        existing = self.with_context(active_test=False).search(
            [('phone_formatted', '=', formatted)], limit=1)
        if existing:
            update = {}
