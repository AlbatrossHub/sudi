...
class OwaBroadcastGroup(models.Model):
    ...
    def fanout(self, body_text, attachments=None, quick_reply=None):
        """Programmatic fan-out: queue one outbound owa.message per
        recipient. Each gets its own 1:1 channel via the existing send
        cron path.

        When ``quick_reply`` is given the body is rendered PER recipient
        (variable substitution + placeholder stripping) instead of using the
        single literal ``body_text`` for everyone."""
        self.ensure_one()
        ...
        for partner in self.partner_ids:
            ...
            if not mobile:
                continue
            try:
                # Per-recipient render when a template is supplied; otherwise
                # the literal body_text.
                body = (quick_reply.render_plain(record=partner)
                        if quick_reply else (body_text or ''))
