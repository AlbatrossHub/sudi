...
class OwaCallLog(models.Model):
    ...
    _uniq_account_call_id = models.Constraint(
    ...
    def _resolve_partner_from_phone(self, phone):
        ...
        for needle in (digits, digits[-10:]):
            if not needle:
                continue
            # NB: Odoo 19 has NO res.partner.mobile field — searching it raises
            # "Invalid field mobile". The v18 tree also matches mobile; v19 is
            # phone-only. (#calls / gotcha #1)
            partner = Partner.search([('phone', 'ilike', needle)], limit=1)
            if partner:
                return partner
        return self.env['res.partner']
    ...
    def action_open_channel(self):
        """Form-action: open the linked WhatsApp conversation in Discuss."""
        self.ensure_one()
        if not self.channel_id:
            return False
        # Open the live Discuss conversation, NOT the discuss.channel backend
        # form (which shows channel settings, not the chat). (#wa-open-chat)
    ...
    def action_mass_block_number(self):
        ...
        for rec in self:
            ...
            if not phone:
                continue
            # add_to_blacklist normalizes the number and dedupes via
            # phone_formatted (incl. reactivating archived rows). owa.blacklist
            # is global (no per-account field), so it is keyed by number only.
            formatted = wa_phone_format(self.env, phone) or phone
            if Blacklist.with_context(active_test=False).search_count(
                    [('phone_formatted', '=', formatted), ('active', '=', True)]):
                continue
            Blacklist.add_to_blacklist(phone, reason=_("Auto-blocked from call log"))
            added += 1
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
