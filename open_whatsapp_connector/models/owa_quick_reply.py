...
class OwaQuickReply(models.Model):
    ...
    def render_plain(self, record=None, free_text_values=None):
        """Render to plain WhatsApp text: substitute variables, drop any
        unresolved ``{{placeholders}}`` so they are never delivered verbatim,
        then strip HTML tags. Used by the standing-order and broadcast
        fan-out paths (mirrors the campaign's render+strip behaviour)."""
        import re
        from html import unescape
        self.ensure_one()
        body = self.render_body(record=record, free_text_values=free_text_values) or ''
        body = re.sub(r'\{\{\s*\w+\s*\}\}', '', body)
        return unescape(re.sub(r'<[^>]+>', '', body)).strip()
