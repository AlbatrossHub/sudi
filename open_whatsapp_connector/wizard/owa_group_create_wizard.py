...
class OwaGroupCreateWizard(models.TransientModel):
    ...
    def action_create(self):
        ...
        if self.wa_account_id.connection_type == 'cloud':
            # Cloud Groups API (2026): create the group, then normalize the
            # Graph response into the same metadata shape the QR path yields.
            # Participants can't be seeded at create time on Cloud — invitees
            # join via the invite link, which we surface in the group record.
            api = self.wa_account_id._get_cloud_api()
            try:
                res = api.create_group(self.subject.strip())
                group_id = res.get('id') or (res.get('groups') or [{}])[0].get('id')
                invite = None
                try:
                    invite = api.group_invite_link(group_id)
                except Exception:
                    _logger.info("Cloud group invite-link fetch failed", exc_info=True)
                metadata = {
                    'id': group_id,
                ...
                _logger.exception("Cloud group creation failed")
                raise UserError(_(
                    "WhatsApp rejected the group creation: %s\n"
                    "Note: the Cloud Groups API requires an Official "
                    "Business Account.") % exc)
        else:
