...
class OwaComposer(models.TransientModel):
    ...
    def _send_standalone(self):
        ...
        # creation work unchanged. When a scheduled_date is set we thread it
        # through context: message_post stamps it on the owa.message and SKIPS
        # the synchronous send, so the send cron transmits it when due. The old
        # post-hoc search for a state='outgoing' row never matched (the message
        # was already 'sent'/'error' by then) — scheduled sends were dropped.
        # (#F102)
        post_channel = channel
        if self.scheduled_date:
            post_channel = channel.with_context(owa_scheduled_date=self.scheduled_date)
        posted = post_channel.message_post(
            body=body,
        ...
        # Also log the sent message on the CONTACT's chatter so it shows in the
        # record's history. Discuss already shows it via the linked channel, but
        # the top-menu Compose path never touched the partner's own thread — the
        # message was invisible from the contact form. Best-effort: a chatter-log
        # failure must never block the send. Only when a real partner was
        # resolved (never for a raw phone with auto-create contacts OFF).
        # (#compose-chatter)
        if partner:
            try:
                who = partner.display_name or formatted
                note = Markup("<p><em>%s</em></p><p>%s</p>") % (
                    _("WhatsApp sent to %s:") % who,
                    body or _("(attachment)"),
                )
                ...
                _logger.exception(
                    "owa.composer: chatter log to partner %s failed", partner.id)

        # Close the dialog and toast the real outcome — same UX as the
        # record-bound send path. Previously this returned an act_window that
        # opened the discuss.channel *settings* form, which rendered as a stray
        # "group" form even for a 1:1 chat (confusing — the user just wanted to
        # send, not configure the channel). For a non-scheduled send,
        # message_post fires _send_message() synchronously, so the linked
        # owa.message state is final here. (#compose-no-redirect)
        owa_msg = self.env['owa.message'].search(
            [('mail_message_id', '=', posted.id)], order='id desc', limit=1)
        recipient = partner.name if partner else formatted
        ntype = 'success'
        if self.scheduled_date:
            title = _("WhatsApp Message Scheduled")
            note = _("Message to %s scheduled.", recipient)
        elif owa_msg.state == 'error':
            ntype = 'warning'
            title = _("WhatsApp Send Failed")
            note = (owa_msg.failure_reason or owa_msg.error_message
                    or _("The message to %s could not be sent.", recipient))
        else:
            title = _("WhatsApp Message Sent")
            note = _("Message sent to %s.", recipient)
        ...
        return {
            ...
            'params': {
                ...
                # Close the compose dialog after the toast.
    ...
    def action_send(self):
        ...
        # Block upfront on a pending self-service account (mirrors the
        # send-queue's _account_messaging_blocked gate) so the user gets a clear
        # message instead of a silently-failed message in the thread. No effect
        # in 'shared' mode.
        if self.wa_account_id._account_messaging_blocked():
            raise UserError(_(
                "WhatsApp account '%s' is pending administrator approval and "
                "cannot send messages yet. Ask a WhatsApp Administrator to "
