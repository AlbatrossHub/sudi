...
class OwaChannelMergeWizard(models.TransientModel):
    ...
    def action_merge(self):
        ...
        # Refuse to interleave histories from two different WhatsApp accounts —
        # the message move below is irreversible. (#F106)
        if source.owa_account_id != target.owa_account_id:
            raise UserError(_(
                "Both conversations must belong to the same WhatsApp account."))
        # Move messages
        self.env['mail.message'].sudo().search([
            ('model', '=', 'discuss.channel'),
            ('res_id', '=', source.id),
        ]).write({'res_id': target.id})
        # Re-point attachments bound to the source channel to the target BEFORE
        # unlink. Otherwise core BaseModel.unlink hard-deletes every
        # ir.attachment with res_model='discuss.channel', res_id=source.id —
        # i.e. all of the merged conversation's inbound media and Discuss
        # uploads — because the message rows that referenced them were just
        # moved away. (#F101)
