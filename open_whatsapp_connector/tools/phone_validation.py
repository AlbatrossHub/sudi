...
def wa_phone_format(env, number, country=None, force_format='E164'):
    ...
    if not number:
        return False
    # WhatsApp JIDs (…@g.us, @newsletter, @broadcast, @lid, @s.whatsapp.net)
    # are not phone numbers. phonenumbers would parse the leading digits as a
    # real number and silently drop the suffix, re-routing group traffic to a
    # bogus personal number. Callers use the ``wa_phone_format(...) or raw``
    # idiom, so returning False preserves the JID. (#jid-not-a-phone)
    if '@' in str(number):
        return False
    if not country:
