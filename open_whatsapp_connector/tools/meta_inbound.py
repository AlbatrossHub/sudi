...
def normalize_meta_message(value, message):
    ...
    if mtype == 'text':
        ...
        if inter.get('type') == 'nfm_reply':
            # WhatsApp Flow submission: surface the human-readable label and
            # append the structured answers (response_json); integrations can
            # read the full payload from ``raw``.
            body = sub.get('body') or sub.get('name') or 'Flow response'
            if sub.get('response_json'):
                body = f"{body}\n{sub['response_json']}"
        else:
