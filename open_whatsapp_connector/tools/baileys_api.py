...
class BaileysApi:
    ...
    def create_session(self, odoo_base_url, webhook_secret, callback_url=None,
                       send_read_receipts=True, share_online_presence=False,
                       capture_contact_statuses=False, status_mark_seen=False,
                       sync_own_statuses=False):
        ...
        :param share_online_presence: when True, sidecar marks the number
            online (``sendPresenceUpdate('available')``) so WhatsApp streams
            contacts' presence back — at the cost of the number appearing
            online to contacts.
        :param capture_contact_statuses: when True, forward contacts' Status
            broadcasts (Stories) to the ``/webhook/status`` endpoint.
        :param status_mark_seen: when True, send a view receipt when a
            captured status is opened in Odoo.
        """
        return self._request('POST', '/session/create', json={
            ...
            'share_online_presence': share_online_presence,
            'capture_contact_statuses': capture_contact_statuses,
            'status_mark_seen': status_mark_seen,
            'sync_own_statuses': sync_own_statuses,
        })

    def update_session_config(self, send_read_receipts=None,
                              share_online_presence=None,
                              capture_contact_statuses=None,
                              status_mark_seen=None,
                              sync_own_statuses=None):
        ...
        if share_online_presence is not None:
            body['share_online_presence'] = share_online_presence
        if capture_contact_statuses is not None:
            body['capture_contact_statuses'] = capture_contact_statuses
        if status_mark_seen is not None:
            body['status_mark_seen'] = status_mark_seen
        if sync_own_statuses is not None:
            body['sync_own_statuses'] = sync_own_statuses
        return self._request('POST', f'/session/{self._account_key()}/config', json=body)

    # ── WhatsApp Status (Stories) ─────────────────────────────────────
    def revoke_status(self, message_id):
        """Delete a status you posted from every contact's phone."""
        return self._request(
            'POST', f'/session/{self._account_key()}/status/revoke',
            json={'message_id': message_id})

    def status_react(self, status_key, emoji):
        """React with an emoji to a contact's status."""
        return self._request(
            'POST', f'/session/{self._account_key()}/status/react',
            json={'status_key': status_key, 'emoji': emoji or ''})

    def status_reply(self, to_jid, status_key, text):
        """Reply to a contact's status — a DM to them quoting the status."""
        return self._request(
            'POST', f'/session/{self._account_key()}/status/reply',
            json={'to': to_jid, 'status_key': status_key, 'text': text or ''})

    def status_mark_seen(self, status_key):
        """Send a view receipt for a contact's status (they see you viewed)."""
        return self._request(
            'POST', f'/session/{self._account_key()}/status/seen',
            json={'status_key': status_key})
    ...
    def edit_message(self, number, message_id, new_text):
        """Edit a previously-sent message in place. Returns the new message_id.

        WhatsApp only allows editing your own recently-sent messages; the
        server enforces a ~15 minute window and rejects older edits.
        """
        data = {
            'account_id': self._account_key(),
            'to': number,
        ...
        result = self._request('POST', '/send/edit', json=data)
        return result.get('message_id', '')
    ...
    def history_arm(self):
        """Arm history capture for the NEXT pairing of this account WITHOUT
        re-linking. Call BEFORE create_session (Connect) so a single fresh-QR
        pairing streams the existing chats/contacts/messages into the sidecar
        buffer during that one scan — the post-connect import then drains the
        buffer with no second scan. Returns ``{'armed': bool}``."""
        return self._request(
            'POST', f'/session/{self._account_key()}/history/arm', json={})

    def history_resync(self):
        """Reconnect the session with history streaming enabled (no QR re-scan)
        so WhatsApp re-streams recent chats/contacts/messages into the sidecar
        buffer. Returns ``{'started': bool, 'error'?: str}``."""
        return self._request(
            'POST', f'/session/{self._account_key()}/history/resync', json={})

    def history_status(self):
        """Return the sidecar history-buffer status for this account:
        ``{available, syncComplete, syncing, connected, progress, counts}``."""
        return self._request(
            'GET', f'/session/{self._account_key()}/history/status')

    def fetch_history_page(self, cursor=0, limit=200):
        """Drain one page of buffered history. Contacts + chats are returned on
        the first page (cursor=0); messages page from ``cursor``. Returns
        ``{contacts, chats, messages, nextCursor, done}``."""
        return self._request(
            'GET',
            f'/session/{self._account_key()}/history'
    ...
    def fetch_profile_picture(self, jid, hi_res=True):
        """Fetch (download) a profile picture FROM WhatsApp for any JID — a
        contact (``<digits>@s.whatsapp.net``), a group (``@g.us``) or a channel
        (``@newsletter``). The sidecar downloads the short-lived CDN URL and
        returns ``{'url', 'image_base64', 'mimetype'}`` — all None when the JID
        has no picture or privacy hides it. ``image_base64`` is ready to write
        straight into a fields.Image (no re-encoding needed)."""
        return self._request(
            'GET',
            f'/session/{self._account_key()}/profile-pic',
            params={'jid': jid, 'hires': '1' if hi_res else '0'},
        )
    ...
    def newsletter_fetch_messages(self, newsletter_jid, count=100, since=0, after=0):
        """Fetch recent posts from a WhatsApp Channel (newsletter). Best-effort:
        channel-post history is not part of the standard history sync and may be
        partial or unsupported on some gateway builds, in which case the sidecar
        returns an empty list. Returns ``{'messages': [...]}``."""
        return self._request(
            'GET',
            f'/session/{self._account_key()}/newsletters/{newsletter_jid}/messages'
            f'?count={int(count)}&since={int(since)}&after={int(after)}')
