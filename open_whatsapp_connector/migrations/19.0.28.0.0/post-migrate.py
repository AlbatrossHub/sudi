# -*- coding: utf-8 -*-
"""post-migrate for open_whatsapp_connector 19.0.28.0.0.

The post_init_hook (_create_default_notification_rules) only runs on a FRESH
install, never on upgrade. v28 broadens that helper from 4 to 10 ready-to-use
...
def migrate(cr, version):
    ...
    try:
        ...
        _logger.info(
            "[owc 19.0.28.0.0] refreshed default notification rules "
            "(added Quotation Sent / PO Confirmed / Repair / CRM / Project / Helpdesk)")
    except Exception:
        _logger.exception(
            "[owc 19.0.28.0.0] failed to refresh default notification rules")
