...
patch(Composer.prototype, {
    ...
    /**
     * Open the small wizard that lets the agent choose which connected
     * WhatsApp account this conversation replies from (multi-account support).
     * Writing the channel's account is enough — the server routes every reply
     * by discuss.channel.owa_account_id. (#reply-account)
     */
    openOwaAccountPicker() {
        this.action.doAction(
            {
                type: "ir.actions.act_window",
                name: _t("Reply from account"),
                res_model: "owa.reply.account.wizard",
                view_mode: "form",
                views: [[false, "form"]],
                target: "new",
                context: {
                    default_channel_id: this.thread.id,
                    default_wa_account_id: this.thread.owa_account_id?.id || false,
                },
            },
            {
                onClose: () => {
