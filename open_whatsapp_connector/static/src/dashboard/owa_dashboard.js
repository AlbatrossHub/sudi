...
export class OwaDashboard extends Component {
    ...
    _emptyData() {
        return {
            counters: {
                ...
                sidecars_total: 0, sidecars_running: 0, sidecar_default_online: false,
    ...
    /** True when the DB has no WhatsApp accounts yet (fresh install). The
     * header button then acts as a Create-Account CTA rather than a
     * sidecar-state pill. (#empty_cta) */
    hasNoAccounts() {
        const c = this.state.data.counters || {};
        return !(c.accounts_total || 0);
    }

    sidecarBtnClass() {
        const base = "btn";
        if (this.hasNoAccounts()) {
            return `${base} owa_btn_sidecar_create`;
        }
    ...
    async onStartSidecars() {
        // Fresh install: no accounts yet. The sidecar can do nothing until an
        // account exists, so route the user straight to account creation
        // instead of the dead-end "No accounts configured." warning. (#empty_cta)
        if (this.hasNoAccounts()) {
    ...
    /** Open a blank WhatsApp account form; refresh the dashboard on close so
     * the new account (and its Connect/QR flow) is reflected immediately. */
    openCreateAccount() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: _t("New WhatsApp Account"),
    ...
    connectedTileSubtext() {
        const c = this.state.data.counters || {};
        if (!c.accounts_total) {
            return c.sidecar_default_online ? _t("sidecar ready") : _t("no accounts");
        }
