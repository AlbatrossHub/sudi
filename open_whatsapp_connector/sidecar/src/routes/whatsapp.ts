...
export function createWhatsappRoutes(manager: SessionManager): Router {
  ...
  router.get(
    "/session/:id/newsletters/:jid/messages",
    async (req: Request, res: Response) => {
      try {
        ...
        const r = await manager.newsletterFetchMessages(
          req.params.id, req.params.jid, count, since, after);
        res.json(r);
      } catch (err: any) { res.status(500).json({ error: err.message }); }
    },
  );
  ...
  // Fetch a profile picture FROM WhatsApp for any JID (contact / group /
  // channel). jid is passed as a query param to avoid URL-encoding the
  // '@' and ':' in path segments. hires=0 fetches the thumbnail.
  router.get("/session/:id/profile-pic", async (req: Request, res: Response) => {
    try {
      const jid = (req.query.jid as string) || "";
      if (!jid) {
        res.status(400).json({ error: "jid query param is required" });
        return;
      }
      const hiRes = req.query.hires !== "0";
      const r = await manager.fetchProfilePicture(req.params.id, jid, hiRes);
      res.json(r);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  // ── WhatsApp Status (Stories) interactions ───────────────────────────
  router.post("/session/:id/status/revoke", async (req: Request, res: Response) => {
    try {
      const { message_id } = req.body || {};
      if (!message_id) { res.status(400).json({ error: "message_id is required" }); return; }
      res.json(await manager.revokeStatus(req.params.id, message_id));
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  router.post("/session/:id/status/react", async (req: Request, res: Response) => {
    try {
      const { status_key, emoji } = req.body || {};
      res.json(await manager.statusReact(req.params.id, status_key, emoji || ""));
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  router.post("/session/:id/status/reply", async (req: Request, res: Response) => {
    try {
      const { to, text } = req.body || {};
      if (!to) { res.status(400).json({ error: "to is required" }); return; }
      res.json(await manager.statusReply(req.params.id, to, text || ""));
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
  router.post("/session/:id/status/seen", async (req: Request, res: Response) => {
    try {
      const { status_key } = req.body || {};
      res.json(await manager.statusMarkSeen(req.params.id, status_key));
    } catch (err: any) { res.status(500).json({ error: err.message }); }
  });
