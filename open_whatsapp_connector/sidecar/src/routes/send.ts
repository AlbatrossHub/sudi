...
export function createSendRoutes(manager: SessionManager): Router {
  ...
  router.post("/send/edit", async (req: Request, res: Response) => {
    try {
      const { account_id, to, message_id, text } = req.body;
      if (!account_id || !to || !message_id || !text) {
        res.status(400).json({ error: "account_id, to, message_id, and text are required" });
        return;
      }
      const result = await manager.editMessage(account_id, to, message_id, text);
      res.json(result);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
