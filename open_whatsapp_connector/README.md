# Open WhatsApp Connector for Odoo 19

**Free WhatsApp integration for Odoo 19 using the WhatsApp Web protocol.**
No Meta Business API approval needed — just scan a QR code to connect.

> Current version: **19.0.27.0.0** — feature-parity with the v18.0.27.0.0 branch.
...
- **Configurable per-user/per-team marketing visibility** — Settings → WhatsApp → Campaign & Contact Visibility (`Shared` / `Restricted to owner + their Sales Team`). Adds `user_id` + `team_id` to campaigns, contact lists, broadcast groups, standing orders, status broadcasts; toggles 12 `ir.rule` + 6 `ir.model.access` rows at runtime. Default = shared (no behavioural change for existing customers).
- **Push-based connection state** — new `/open_whatsapp_connector/webhook/connection` route; sidecar emits transitions (connecting → qr_pending → connected, all disconnect variants) so the dashboard flips state within ~200 ms of QR-scan instead of waiting for the 5-min heartbeat cron.
- **Chat-window header avatar fix** — header "Thread Image" now correctly resolves through `correspondent.persona.avatarUrl` (was rendering with `src=null`).

### New in v19.0.26

- **Incoming call awareness** — voice + video calls log to `owa.call.log`,
  missed-call auto-reply, click-to-call buttons on every partner,
  "Missed calls 24h" KPI tile on the dashboard
- **Customer-360 timeline** — one OWL view per contact aggregating
  WhatsApp messages + calls + sale orders + invoices + helpdesk tickets +
  CRM leads + activities, chronologically sorted
- **Inbound auto-create rules** — regex / keyword / sender-pattern matchers
  create CRM leads, helpdesk tickets, sale-order drafts, project tasks
  or hr applicants from incoming messages
- **Conversation triage** — assignee, SLA timer, breach activities,
  reopen-on-inbound, resolved-state markers
- **Helpdesk + CSAT bridge** — discuss-channel ↔ helpdesk-ticket linkage,
  auto-send a CSAT survey when a thread resolves
- **Broadcast Groups** — saved recipient lists with per-recipient session
  isolation
- **Standing Orders** — recurring scheduled sends (daily/weekly/monthly)
- **Status Broadcasts** — post WhatsApp Stories with 24-hour expiry
- **Conversation labels** — color-coded multi-tag triage
- **Auto-tag rules** — regex/keyword rules apply `res.partner.category` tags
  on first inbound
- **Outbound throttling** — per-account msgs/min/hour/day caps with
  anti-ban backoff
- **Opt-in tracking** — `wa_is_opted_in` / `wa_opt_in_at` / `wa_opt_in_source`
  on `res.partner` for compliant outbound
- **12 ready-to-use sales templates** — Quote / Order / Invoice / Refund flow
- **Channel merge wizard** — combine duplicate WhatsApp channels
- **Mass actions on the Calls log** — Create Lead / Block Number
- **Partner avatar sync** — daily cron pulls WhatsApp profile pictures into
...
│  Odoo 19         │ <───────────────>  │  Node.js Sidecar │ <──────────────> WhatsApp Servers
...
├── __manifest__.py                # version 19.0.27.0.0
├── __init__.py
├── controllers/
...
├── migrations/
│   ├── 19.0.2.0.0/post-migrate.py     # Phase 0-11 schema sanity
│   ├── 19.0.26.1.0/post-migrate.py    # PDF defaults backfill
│   └── 19.0.27.0.0/post-migrate.py    # user_id/team_id backfill on 5 marketing models
