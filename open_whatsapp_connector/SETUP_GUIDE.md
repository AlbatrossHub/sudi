...
| Requirement | Details |
|-------------|---------|
| **Odoo (17, 18 or 19)** | Community or Enterprise edition, already installed and running. Use the version that matches this module's build — a `17.0.x` package needs Odoo 17, `18.0.x` needs Odoo 18, and so on (see `version` in `__manifest__.py`). |
| **Node.js 22+** | Download from [nodejs.org](https://nodejs.org/) |
| **Python `phonenumbers`** | Required by Odoo's phone features (the module won't install without it). Most Odoo servers already have it; if install complains it's missing, run `sudo apt install python3-phonenumbers` (Linux) or `pip install phonenumbers`. |
...
> **If you copied the module from a Windows machine (or a manually built ZIP)** and `./setup.sh`
> fails with `cannot execute: required file not found` or `$'\r': command not found`, the script
> picked up Windows line endings. Convert it to Unix line endings once, then re-run:
> ```bash
> sed -i 's/\r$//' setup.sh
> ./setup.sh
> ```
> (Downloads via the Odoo Apps Store / `git clone` are not affected.)
...
You should see a line like (the JSON also includes pino metadata such as `level`/`time`/`pid`):
```
{"level":30,"time":1718000000000,"pid":12345,"hostname":"your-server","port":3100,"msg":"WhatsApp sidecar started"}
```
...
>
> **Running more than one Odoo instance on the same server?** Give each its own sidecar: copy this
> service file under a unique name (e.g. `whatsapp-sidecar-prod.service`), point `WorkingDirectory`
> at that instance's sidecar folder, and set a unique `Environment=PORT=` (e.g. `3101`, `3102`, …).
> Then set each Odoo account's **Sidecar URL** to match its port (e.g. `http://localhost:3102`).
...
2. **Sidecar Directory** is auto-filled on install — leave it (only change if you moved the sidecar)
...
| **Sidecar Directory** | **Auto-filled on install** — leave as-is. Only change it if you moved the sidecar to a different folder/server. | (auto-detected, e.g. `YOUR_ADDONS_PATH/open_whatsapp_connector/sidecar`) |
...
3. Choose a **Connection Type**:
   - **QR / Device Link** (default) — links a personal or Business WhatsApp number by scanning a QR code, via the bundled sidecar. No Meta account needed. **This guide covers this method.**
   - **Official Cloud API** — for Meta WhatsApp Business API numbers. The sidecar is **not** used; instead you paste your Meta credentials (Phone Number ID, WABA ID, App ID/Secret, Access Token), copy the webhook Callback URL + Verify Token into the Meta console, and click **Verify Connection**. If you pick this, skip the QR steps below and continue from Step 6.
4. Leave the other fields at their defaults and **Save**.

> **Steps 5.3–5.5 apply to QR / Device Link accounts.** Cloud API accounts connect via **Verify Connection** instead and have no sidecar/QR step.
...
### 5.6 Import your existing WhatsApp data (optional)

Once an account is **Connected**, you can pull your existing WhatsApp groups, contacts and recent
conversations into Odoo.

1. Go to **WhatsApp > Chats > Import Data**
2. Pick the connected **WhatsApp Account**, then choose what to import:

| Option | What it does | How it runs |
|--------|--------------|-------------|
| **Groups** | Creates a conversation for each WhatsApp group you're in | Instantly |
| **Contacts** | Creates Odoo contacts for the people in those groups | Instantly |
| **Chat history** | Imports your one-to-one conversations and their **recent** messages | In the **background**, with a progress bar |

3. Click **Import**.

> **About "Chat history":** it runs in the background so it never floods or freezes Odoo. It brings
> **recent history only** — not a full lifetime archive — because that's all WhatsApp streams over the
> QR (device-link) connection. Profile photos fill in over time as people message you.

#### Watch the progress

If you selected **Chat history**, Odoo opens an import job with a live progress bar. You can also
review past and running imports any time at **WhatsApp > Chats > Import Jobs** (status, progress, and
how many contacts / conversations / messages were imported).

> **Re-running is safe.** Imports are idempotent — running an import again won't create duplicate
...
The module ships with a library of pre-built notification rules (all **disabled by default**). They automatically send WhatsApp messages when business events happen. Rules are only created for the apps you actually have installed, so you may see fewer than the full list below.

> **Installed a business app *after* the connector?** Rules are seeded for the apps that are present when the connector is installed. If you add **Sales**, **Accounting**, **Inventory** (and so on) later, open **Apps**, find **Open WhatsApp Connector** and click **Upgrade** — that refreshes the rule library for the newly installed apps.

### 7.1 View the rules

Go to **WhatsApp > Configuration > Notification Rules**

**State-based rules** — fire reliably; just pick an account and activate:

| Rule | When it fires |
|------|--------------|
| Sale Order Confirmed | When a sale order is confirmed |
| Quotation Sent | When a quotation is sent |
...
| Purchase Order Confirmed | When a purchase order is confirmed |
| Repair Completed | When a repair order is marked done |

**Stage-based rules** — the trigger stage name varies per database, so open the rule and confirm its **Trigger Value** matches your actual stage name *before* activating:

| Rule | When it fires |
|------|--------------|
| CRM — Opportunity Won | When a lead reaches your "Won" stage |
| Project — Task Done | When a task reaches your "Done" stage |
| Helpdesk — Ticket Solved | When a ticket reaches your "Solved" stage |

> **Helpdesk integration needs Odoo Enterprise.** Helpdesk is an Enterprise-only app, so the *Helpdesk — Ticket Solved* rule and the WhatsApp ⇄ Helpdesk two-way bridge are available only on Odoo Enterprise. Every other rule and integration in this guide works on both Community and Enterprise.

> **Shortcut:** In **Settings > Open WhatsApp Connector**, click **"Enable recommended automations"** to activate all the state-based rules at once and assign your connected account. It intentionally skips the stage-based rules (those need the stage-name check above).
...
**From Odoo:**
Go to WhatsApp > Configuration > Accounts > click **Stop Sidecar** then **Start Sidecar**
