...
class OwaWebhook(http.Controller):
    ...
    def webhook_incoming(self, **kwargs):
        ...
        for msg_data in messages:
            try:
                # Savepoint so a failed message rolls back atomically —
                # INCLUDING its dedupe claim. Without it the claim commits
                # with the request and the sidecar's retry is rejected as a
                # duplicate: the message is silently lost. It also keeps one
                # poisoned message from aborting the rest of the batch.
    ...
    def _process_inbound_message(self, account, msg_data):
        ...
        from_me = bool(msg_data.get('from_me'))

        if not from_number:
            return

        # Own-device messages — sent from the phone / another linked device, or
        # the echo of a message Odoo itself sent — are synced as OUTBOUND in
        # _handle_inbound. We don't sync our own reactions in v1, and own
        # messages bypass the inbound access-control below. (#own-device-sync)
        if from_me and msg_type == 'reaction':
            return
        ...
        if msg_type == 'reaction':
            ...
            request.env['owa.message'].sudo()._apply_inbound_reaction(
                target_msg_uid, sender_jid, emoji, account=account,
            )
            return
        ...
        if (not from_me and chat_type == 'direct' and not self._dm_allowed(
            ...
            return
        if not from_me and chat_type == 'group' and not self._group_allowed(account, from_number):
            _logger.info(
        ...
        def _save_media(kind, media_obj):
            ...
            # WhatsApp voice notes arrive as OGG/Opus audio. Flag them with
            # Odoo's voice metadata so Discuss renders the inline VoicePlayer
            # (waveform + play button) instead of a download-only card — same
            # UX as a natively-recorded voice message, and it plays in-browser.
            # `_set_voice_metadata` is provided by the mail/discuss layer; guard
            # in case it is unavailable.
            if kind == 'audio' and hasattr(attachment, '_set_voice_metadata'):
                try:
        ...
        if msg_type == 'text' and msg_data.get('text'):
            ...
            # Build as Markup (escaping the dynamic values) so message_post
            # renders it as HTML instead of literal &lt;details&gt; text, while
            # still escaping the user-supplied name/address. (#F033)
            details = Markup(
                "<details><summary>📍 Location shared</summary>"
                "<div>Lat: {lat}, Lng: {lng}</div>"
            ).format(lat=lat, lng=lng)
            if loc.get('name'):
                details += Markup("<div>Name: {}</div>").format(loc['name'])
            if loc.get('address'):
                details += Markup("<div>Address: {}</div>").format(loc['address'])
            ...
            if att_id:
                ...
                body = Markup("{ph} <em>(content unavailable)</em>").format(
                    ph=_media_placeholder('sticker'))
            ...
            for contact in contacts:
                ...
                for line in vcard.split('\n'):
                    if line.startswith('TEL'):
                        ...
                        break
                # Escape the vCard-supplied name/phone (untrusted). (#F033)
                if phone:
                    rows += Markup("<li>{name} — {phone}</li>").format(
                        name=display_name, phone=phone)
                else:
                    rows += Markup("<li>{name}</li>").format(name=display_name)
            body = Markup(
                "<details><summary>📇 Contact card</summary>"
                "<ul>{rows}</ul></details>"
            ).format(rows=rows)
        ...
        if media_kind:
            ...
            if att_id:
                ...
                # be stored.
                body = Markup("{ph} <em>(content unavailable)</em>").format(
                    ph=_media_placeholder(media_kind))

        # Status-reply context: when a customer replies to one of our Status
        # updates (or any quoted photo/video), the sidecar forwards the quoted
        # media so the agent can see WHAT the customer is asking about (e.g.
        # "how much is this?"). Save it as an extra attachment and prepend a
        # short "in reply to" marker so the conversation isn't contextless.
        quoted_media = context_data.get('quoted_media')
        if quoted_media and quoted_media.get('base64'):
            q_att = _save_media('quoted', quoted_media)
            if q_att:
                attachment_ids.append(q_att)
                note = Markup("<em>%s</em>") % _("↩️ In reply to your status / post:")
                body = (note + Markup("<br/>") + body) if body else note
        ...
        if not body and not attachment_ids:
            ...
            if msg_type in ('image', 'video', 'audio', 'document', 'sticker'):
                body = Markup(
                    "{ph} <em>(couldn't be downloaded — check sidecar network "
                    "access to mmg.whatsapp.net)</em>"
                ).format(ph=_media_placeholder(msg_type))
            else:
    ...
    def webhook_call(self, **kwargs):
        ...
        for call_payload in calls:
            try:
                # Savepoint: a duplicate call event violating the
                # (account, call_id) unique constraint must not abort the
                # transaction for the remaining events in the batch.
                with request.env.cr.savepoint():
    ...
    @http.route('/open_whatsapp_connector/webhook/story', methods=['POST'],
                type='json', auth='public', csrf=False)
    def webhook_story(self, **kwargs):
        """Receive a contact's WhatsApp Status (Story) captured by the sidecar
        — only when the account opted in via ``capture_contact_statuses``.

        Payload: {secret, account_id, message_id, participant, from,
        sender_name, media_type, text, attachment_b64, mimetype, timestamp}.
        Idempotent on (account, message_id)."""
        secret = kwargs.get('secret')
        if not secret:
            return {'status': 'error', 'message': 'Missing secret'}
        account = request.env['owa.account'].sudo().search([
        ...
        if not account:
            _logger.warning("webhook_story: invalid secret")
            return {'status': 'error', 'message': 'Invalid secret'}
        try:
            # Savepoint so a single malformed story never aborts the request.
            with request.env.cr.savepoint():
                if kwargs.get('from_me'):
                    # A Status the user posted from their phone → My Status.
                    request.env['owa.status.broadcast'].sudo()._ingest_own_status(
                        account, kwargs)
                else:
                    request.env['owa.status.update'].sudo()._ingest_status_update(
                        account, kwargs)
        except Exception as e:  # pragma: no cover — per-item safety
            _logger.exception("webhook_story: error ingesting status")
    ...
    def webhook_status(self, **kwargs):
        ...
        for status_data in statuses:
            ...
            if msg_uid and status:
                try:
                    # Per-status savepoint: one malformed receipt must not
                    # 500 the whole batch into a sidecar retry storm.
                    with request.env.cr.savepoint():
                        WaMessage._process_status_update(
                            msg_uid, status, account=account)
                except Exception:
                    _logger.exception(
                        "webhook_status: error applying status %s for %s",
                        status, msg_uid)
    ...
    def webhook_connection(self, **kwargs):
        ...
        try:
            # Write as OdooBot, not the cookieless public-route user, so the
            # tracked session_state change-note in the account chatter is
            # attributed to "OdooBot" rather than "Public user". (#connect_flow)
            account.with_user(SUPERUSER_ID).write(vals)
        except Exception:
            _logger.exception("webhook_connection: write failed for %s",
                              account.id)
            return {'status': 'error', 'message': 'Write failed'}
        if state == 'connected':
            # Best-effort: show the account's real WhatsApp photo on the form
            # (only fills an EMPTY image, never raises). (#self-pic-autofill)
            account.with_user(SUPERUSER_ID)._autofill_profile_picture()
        try:
            ...
            # Push to the account's notify list AND the user who created the
            # account. The person setting up a fresh account is usually not on
            # the notify list yet, but is the one watching the QR and waiting
            # for the form to flip to "Connected". Without this the live update
            # only reached notify_user_ids, so the creator fell back to the 3s
            # poll — which felt like "I had to click Connect again after the
            # scan". (#connect_flow)
            recipients = account.notify_user_ids
            if account.create_uid:
                recipients |= account.create_uid
