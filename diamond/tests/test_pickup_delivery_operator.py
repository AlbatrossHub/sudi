...
class TestSudiPickupDeliveryOperator(TestStockCommon):
    ...
    def test_operator_menus_are_bound_to_internal_users(self):
        pickup_group = self.env.ref("diamond.group_sudi_pickup_app")
        ...
        self.assertIn(pickup_group, pickup_menu.groups_id)
    ...
    def test_pickup_notify_user_is_granted_pickup_app_group(self):
        self.assertFalse(self.notify_user.has_group("diamond.group_sudi_pickup_app"))
        self.notify_user.write({"sudi_notify_pickup_scheduled": True})
        self.assertTrue(self.notify_user.has_group("diamond.group_sudi_pickup_app"))
        self.notify_user.write({"sudi_notify_pickup_scheduled": False})
        self.assertFalse(self.notify_user.has_group("diamond.group_sudi_pickup_app"))
